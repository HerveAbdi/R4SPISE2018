...
Make sure that you start this analysis a new `Rproject` so that
the default directory will be correctly set.

Before we start the analysis,
we need to have our standard packages installed (some from from `Github`) 
and the corresponding libraries loaded:

* `Distatis` (from Guthub)
* `ExPosition`
* `InPosition`
* `PTCA4CTA` (from Guthub)
* `R4SPISE2018`(from Guthub)

...
At the intersection of the row and a column we have the 
...
```{r resdSortingData}
rawData <- read.df.excel(path = path2file, 
                                         sheet = 'Data')$df.data
```

The data are noew read and stored in
...
In this example we will analyze the answers for two sets of questions.
...
and it comprises the
 ...
 with two values correct-incorrect
 
the second set 
comprises the questions `Q15_C01` to `Q38_C24`
(or 32 to 55);
these questions
evaluates the behaviors, opinions, or attitudes of
...
In MCA the data need to recoded as nominal variables
with the constraint that the different levels of a variable
are roughly balanced.
When are variable are naturally nominal, the process is 
...
We use `car::recode` to recode sex as 1.2 to m/f
```{r recodeSex}
...
We will also recode the variable `Age` into four categories
```{r rescaleAge}
...
```{r , fig.cap = 'Response distribution for Lickert scales \\label{tab:Lickert}', outwidth = '100%'}
# 
knitr::kable( t(apply(rawData[,32:55],2,function(x){summary(as.factor(x))})))
```

This table shows that the responses are not very balanced,
...
Because Dimension 1 was mostly separating the inhabitants
of Lille from the inhabitants of Angers, we decided to
perform a subset analysis amd analyze separately 
...
Here we will color the observations by City, to confirm that this is
main source of variation for this data set.
...
```{r plotaMap.A, fig.width= 8 , fig_width = 10}
#pdf('AngersFj.pdf')
...
```{r plotaMap.L, fig.width= 8 , fig_width = 10}
pdf('LilleFj.pdf')

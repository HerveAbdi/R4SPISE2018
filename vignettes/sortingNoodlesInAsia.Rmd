...
Figure above. 
The sorting data are stored in the `sheet` called `r sheetName4Data`
...
in the data frames called (respectively) `multisSort` and
`vocabulary`.
...
(i.e., the data frame `DistanceCube`) is used
...
In the first map shows the scree plot of the eigenvalues of
...
The values of the means of the assessors from the `r k` groups of Judges 
...
```{r kmeans.graph, fig.height=6, fig.width= 9}
